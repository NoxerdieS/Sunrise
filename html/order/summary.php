...
$sql = 'SELECT 
        ...
        FROM order_details 
        INNER JOIN shipping ON order_details.shipping_id = shipping.id 
...
$cost_of_products = $order_info['total'] - $order_info['shipping_cost'] - ($order_info['total'] * $order_info['payment_cost']);
$payment_cost = $order_info['total'] * $order_info['payment_cost'];
  ...
  <main class="cart summary">
    ...
    <div class="order__delivery summary__infoBox">
      <h3>Podsumowanie zamówienia</h3>
        <p><span>Koszt produktów: </span><?= number_format($cost_of_products, 2) ?> zł</p>
        <p><span>Koszt dostawy: </span><?= number_format($order_info['shipping_cost'], 2) ?> zł</p>
        <p><span>Koszt płatności: </span><?= number_format($payment_cost, 2) ?> zł</p>
        <p><span>Łączna kwota: </span><?= number_format($order_info['total'], 2) ?> zł</p>
        <p><span>Sposób dostawy: </span><?= htmlspecialchars($order_info['shipper_name']) ?></p>
        <p><span>Płatność: </span><?= htmlspecialchars($order_info['payment_name']) ?></p>
    </div>
    <div class="order__delivery summary__infoBox">
      <?php
